...
filenames <- sprintf("../../data/cox_%d.csv", 101:150)
filenames_indep <- sprintf("../../data/cox_%d.csv", 201:250)
data_files <- tar_map(
  list(filenames = filenames, seed = 101:150),
  names = "seed",
  tar_target(file, filenames, format = "file")
)
data_files_indep <- tar_map(
  list(filenames = filenames_indep, seed = 201:250),
  names = "seed",
  tar_target(file, filenames, format = "file")
)
barts_targets <- tar_map(
  values = list(
    file = rlang::syms(paste0("file_", 101:150)),
    file_indep = rlang::syms(paste0("file_", 201:250)),
    seed = 101:150
  ),
  names = "seed",
...
list(
  data_files,
  data_files_indep,
  tar_target(data_file_ref, "../../data/cox_12345.csv", format = "file"),
  barts_targets,
