...
wei.tree(X.train,Y.train,X.test,n.iter,sigma.prior.bart=10,ntree,tau.zero=1,k.zero=1,sigma.jump.omega=1,sigma.jump.theta=1,init.theta=NULL,burn.in=1,thinning=1)
...
tau.zero
Shape hyperparameter for the prior distribution of tau.

k.zero
Scale hyperparameter for the prior distribution of tau.

sigma.jump.omega
"Jump" for the proposal distribution of omega used in the Metropolis-Hastings algorithm.

sigma.jump.theta
"Jump" for the proposal distribution of theta used in the Metropolis-Hastings algorithm.

init.theta
Initial value for the theta posterior distribution. Default is set to the MLE, however the user is allowed to experiment another values.
...
$parameters:
	...
	theta
	A vector with (n.iter after burning-in and thinning) rows corresponding to draws from the posterior distribution of theta.

	theta.jump.mat
	A vector with (n.iter after burning-in and thinning) rows corresponding the probability of accepting the proposal distribution in each draw. 
...
#modelling
source("wei.tree.R",echo=FALSE)
wei.tree.out<-wei.tree(X.train=X.train,Y.train=Y.train,X.test=X.test,n.iter=500,ntree=5,burn.in=250,thinning=5)

#FDR
source("FDR.R",echo=FALSE)
FDR.out<-FDR(tree.output=wei.tree.out,FDR=0.05,plot=TRUE)
