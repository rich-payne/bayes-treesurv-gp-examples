...
filenames <- sprintf("../../data/true_tree_%d.csv", 1:50)
filenames_indep <- sprintf("../../data/true_tree_%d.csv", 101:150)
data_files <- tar_map(
  list(filenames = filenames, seed = 1:50),
  names = "seed",
...
barts_targets <- tar_map(
  values = list(
    file = rlang::syms(paste0("file_", 1:50)),
...
list(
  data_files,
  data_files_indep,
  tar_target(data_file_ref, "../../data/true_tree_12345.csv", format = "file"),
  barts_targets,
