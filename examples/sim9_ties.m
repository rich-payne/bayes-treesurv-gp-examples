...
y = round(y, 1);
% Do some censoring
cens = round(exprnd(censparm,N,1), 1);
ind = y <= cens;
y(~ind) = cens(~ind);
Y = [y,double(ind)];
X = dat;
...
for rep = lowrep:highrep
    basename = strcat(['../output/sim',num2str(simnum),'final_N',num2str(N),'_ties_rep']);
    ...
    if rep > 1
        ...
        Tree_Surv(Y,X,'nmcmc',MCMC,'burn',0,'filepath',fname,...
            'seed',rep * 100,'bigK',Kvals,'nprint',nprint,'saveall',1,...
...
bname = '../output/sim9final_N1000_ties_rep';
nrep = 1;
